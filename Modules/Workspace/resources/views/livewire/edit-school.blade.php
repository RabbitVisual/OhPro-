                ...
                <div class="px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                            <h3 class="text-lg font-semibold leading-6 text-slate-900 dark:text-white" id="modal-title">Editar Escola</h3>
                            <div class="mt-2 space-y-4">

                                @if(!$confirmingDeletion)
                                    <div>
                                        <label for="edit-school-name" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Nome da Escola</label>
                                        <div class="mt-2">
                                            <input type="text" wire:model="name" id="edit-school-name" class="block w-full rounded-md border-0 py-1.5 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-slate-800 sm:text-sm sm:leading-6">
                                        </div>
                                        @error('name') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                    </div>

                                    <div>
                                        <label for="edit-school-color" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Cor de Identificação</label>
                                        <div class="mt-2 flex items-center gap-3">
                                            <input type="color" wire:model="color" id="edit-school-color" class="h-10 w-14 rounded border border-slate-300 dark:border-slate-700 bg-transparent p-1 cursor-pointer">
                                        ...
                                        <div class="flex">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <div class="mt-2 text-sm text-red-700 dark:text-red-300">
                                                    <p>Você está prestes a excluir esta escola e <strong>TODAS</strong> as suas turmas, alunos e dados vinculados. Esta ação não pode ser desfeita.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="flex flex-row-reverse gap-3 w-full sm:w-auto">
                        ...
                        @else
                            <button type="button" wire:click="delete" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:w-auto transition-all disabled:opacity-50" wire:loading.attr="disabled">
                                <span wire:loading.remove>Sim, Excluir Definitivamente</span>
