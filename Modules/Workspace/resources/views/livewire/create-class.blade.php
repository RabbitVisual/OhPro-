            ...
            <div x-show="open"
                ...
                <div class="px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/30 sm:mx-0 sm:h-10 sm:w-10">
                            <x-icon name="chalkboard" class="h-6 w-6 text-indigo-600 dark:text-indigo-400" />
                        </div>
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                            <h3 class="text-lg font-semibold leading-6 text-slate-900 dark:text-white" id="modal-title">Nova Turma</h3>
                            <div class="mt-2 space-y-4">
                                <p class="text-sm text-slate-500 dark:text-slate-400">
                                    Adicione uma nova turma à escola selecionada.
                                </p>
                                ...
                                @error('schoolId')
                                    <div class="rounded-md bg-amber-50 dark:bg-amber-900/20 p-4">
                                        <p class="text-amber-800 dark:text-amber-200 text-sm">{{ $message }}</p>
                                    </div>
                                @enderror

                                <div>
                                    <label for="class-name" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Nome da Turma</label>
                                    <div class="mt-2">
                                        <input type="text" wire:model="name" id="class-name" class="block w-full rounded-md border-0 py-1.5 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-slate-800 sm:text-sm sm:leading-6" placeholder="Ex: 6º Ano A">
                                    </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label for="grade" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Série/Ano</label>
                                        <div class="mt-2">
                                            <select wire:model="grade_level" id="grade" class="block w-full rounded-md border-0 py-1.5 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-slate-800 sm:text-sm sm:leading-6">
                                        ...
                                        @error('grade_level') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <label for="subject" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Disciplina</label>
                                        <div class="mt-2">
                                            <input type="text" wire:model="subject" id="subject" class="block w-full rounded-md border-0 py-1.5 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-slate-800 sm:text-sm sm:leading-6" placeholder="Ex: Matemática">
                                        </div>
                                ...
                                <div>
                                    <label for="class-color" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-300">Cor</label>
                                    <div class="mt-2 flex items-center gap-3">
                                        <input type="color" wire:model="color" id="class-color" class="h-10 w-14 rounded border border-slate-300 dark:border-slate-700 bg-transparent p-1 cursor-pointer">
                ...
                <div class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    <button type="button" wire:click="save" class="inline-flex w-full justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 sm:ml-3 sm:w-auto transition-all disabled:opacity-50 disabled:cursor-not-allowed" wire:loading.attr="disabled">
                        <span wire:loading.remove>Criar Turma</span>
